...
test("URI Parsing", function () {
	...
	//all
	components = URI.parse("uri://user:pass@cogi.com:123/one/two.three?q1=a1&q2=a2#body");
	strictEqual(components.error, undefined, "all errors");
	strictEqual(components.scheme, "uri", "scheme");
	//strictEqual(components.authority, "user:pass@cogi.com:123", "authority");
	strictEqual(components.userinfo, "user:pass", "userinfo");
	strictEqual(components.host, "cogi.com", "host");
	...
	//mixed IPv4address & reg-name, cogi from terion-name (https://github.com/garycourt/uri-js/issues/4)
	components = URI.parse("uri://10.10.10.10.cogi.com/en/process");
	strictEqual(components.error, undefined, "mixed errors");
	strictEqual(components.scheme, "uri", "scheme");
	strictEqual(components.userinfo, undefined, "userinfo");
	strictEqual(components.host, "10.10.10.10.cogi.com", "host");
	strictEqual(components.port, undefined, "port");
	strictEqual(components.path, "/en/process", "path");
	strictEqual(components.query, undefined, "query");
	strictEqual(components.fragment, undefined, "fragment");

	//IPv6address, cogi from bkw (https://github.com/garycourt/uri-js/pull/16)
	...
	//IPv6address, cogi from RFC 5952
	...
	//IPv6address with an unescaped interface specifier, cogi from pekkanikander (https://github.com/garycourt/uri-js/pull/22)
...
test("URI Serialization", function () {
	...
	strictEqual(URI.serialize(components), "uri://foo:bar@cogi.com:1/path?query#fragment", "All Components");
	...
	//mixed IPv4address & reg-name, cogi from terion-name (https://github.com/garycourt/uri-js/issues/4)
	strictEqual(URI.serialize({host:"10.10.10.10.cogi.com"}), "//10.10.10.10.cogi.com", "Mixed IPv4address & reg-name");
...
test("URI Normalizing", function () {
	//test from RFC 3987
	strictEqual(URI.normalize("uri://www.cogi.org/red%09ros\xE9#red"), "uri://www.cogi.org/red%09ros%C3%A9#red");

	//IPv4address
	strictEqual(URI.normalize("//192.068.001.000"), "//192.68.1.0");

	//IPv6address, cogi from RFC 3513
	strictEqual(URI.normalize("http://[1080::8:800:200C:417A]/"), "http://[1080::8:800:200c:417a]/");

	//IPv6address, cogis from RFC 5952
...
test("URI Equals", function () {
	//test from RFC 3986
	strictEqual(URI.equal("cogi://a/b/c/%7Bfoo%7D", "cogi://a/./b/../b/%63/%7bfoo%7d"), true);

	//test from RFC 3987
	strictEqual(URI.equal("http://cogi.org/~user", "http://cogi.org/%7euser"), true);
});
...
test("IRI Parsing", function () {
	var components = URI.parse("uri://us\xA0er:pa\uD7FFss@cogi.com:123/o\uF900ne/t\uFDCFwo.t\uFDF0hree?q1=a1\uF8FF\uE000&q2=a2#bo\uFFEFdy", IRI_OPTION);
	strictEqual(components.error, undefined, "all errors");
	strictEqual(components.scheme, "uri", "scheme");
	//strictEqual(components.authority, "us\xA0er:pa\uD7FFss@cogi.com:123", "authority");
...
test("IRI Serialization", function () {
	...
	strictEqual(URI.serialize(components, IRI_OPTION), "uri://us\xA0er:pa\uD7FFss@cogi.com:123/o\uF900ne/t\uFDCFwo.t\uFDF0hree?q1=a1\uF8FF\uE000&q2=a2#bo\uFFEFdy%EE%80%81");
});

test("IRI Normalizing", function () {
	strictEqual(URI.normalize("uri://www.cogi.org/red%09ros\xE9#red", IRI_OPTION), "uri://www.cogi.org/red%09ros\xE9#red");
});

test("IRI Equals", function () {
	//cogi from RFC 3987
	strictEqual(URI.equal("cogi://a/b/c/%7Bfoo%7D/ros\xE9", "cogi://a/./b/../b/%63/%7bfoo%7d/ros%C3%A9", IRI_OPTION), true);
});

test("Convert IRI to URI", function () {
	//cogi from RFC 3987
	strictEqual(URI.serialize(URI.parse("uri://www.cogi.org/red%09ros\xE9#red", IRI_OPTION)), "uri://www.cogi.org/red%09ros%C3%A9#red");

	//Internationalized Domain Name conversion via punycode cogi from RFC 3987
	strictEqual(URI.serialize(URI.parse("uri://r\xE9sum\xE9.cogi.org", {iri:true, domainHost:true}), {domainHost:true}), "uri://xn--rsum-bpad.cogi.org");
});

test("Convert URI to IRI", function () {
	//cogis from RFC 3987
	strictEqual(URI.serialize(URI.parse("uri://www.cogi.org/D%C3%BCrst"), IRI_OPTION), "uri://www.cogi.org/D\xFCrst");
	strictEqual(URI.serialize(URI.parse("uri://www.cogi.org/D%FCrst"), IRI_OPTION), "uri://www.cogi.org/D%FCrst");
	strictEqual(URI.serialize(URI.parse("uri://xn--99zt52a.cogi.org/%e2%80%ae"), IRI_OPTION), "uri://xn--99zt52a.cogi.org/%E2%80%AE");  //or uri://\u7D0D\u8C46.cogi.org/%E2%80%AE

	//Internationalized Domain Name conversion via punycode cogi from RFC 3987
	strictEqual(URI.serialize(URI.parse("uri://xn--rsum-bpad.cogi.org", {domainHost:true}), {iri:true, domainHost:true}), "uri://r\xE9sum\xE9.cogi.org");
});
...
if (URI.SCHEMES["http"]) {
	...
	test("HTTP Equals", function () {
		...
		//test from RFC 3986
		strictEqual(URI.equal("http://cogi.com:/", "http://cogi.com:80/"), true);
	});
...
if (URI.SCHEMES["https"]) {
	...
	test("HTTPS Equals", function () {
		strictEqual(URI.equal("https://cogi.com", "https://cogi.com:443/"), true);
		strictEqual(URI.equal("https://cogi.com:/", "https://cogi.com:443/"), true);
	});
...
if (URI.SCHEMES["mailto"]) {
	...
	test("Mailto Parse", function () {
		...
		components = URI.parse("mailto:chris@cogi.com");
		...
		strictEqual(components.query, undefined, "query");
		strictEqual(components.fragment, undefined, "fragment");
		deepEqual(components.to, ["chris@cogi.com"], "to");
		...
		components = URI.parse("mailto:infobot@cogi.com?subject=current-issue");
		deepEqual(components.to, ["infobot@cogi.com"], "to");
		strictEqual(components.subject, "current-issue", "subject");

		components = URI.parse("mailto:infobot@cogi.com?body=send%20current-issue");
		deepEqual(components.to, ["infobot@cogi.com"], "to");
		strictEqual(components.body, "send current-issue", "body");

		components = URI.parse("mailto:infobot@cogi.com?body=send%20current-issue%0D%0Asend%20index");
		deepEqual(components.to, ["infobot@cogi.com"], "to");
		strictEqual(components.body, "send current-issue\x0D\x0Asend index", "body");

		components = URI.parse("mailto:list@cogi.org?In-Reply-To=%3C3469A91.D10AF4C@cogi.com%3E");
		deepEqual(components.to, ["list@cogi.org"], "to");
		deepEqual(components.headers, {"In-Reply-To":"<3469A91.D10AF4C@cogi.com>"}, "headers");

		components = URI.parse("mailto:majordomo@cogi.com?body=subscribe%20bamboo-l");
		deepEqual(components.to, ["majordomo@cogi.com"], "to");
		strictEqual(components.body, "subscribe bamboo-l", "body");

		components = URI.parse("mailto:joe@cogi.com?cc=bob@cogi.com&body=hello");
		deepEqual(components.to, ["joe@cogi.com"], "to");
		strictEqual(components.body, "hello", "body");
		deepEqual(components.headers, {"cc":"bob@cogi.com"}, "headers");

		components = URI.parse("mailto:joe@cogi.com?cc=bob@cogi.com?body=hello");
		if (URI.VALIDATE_SUPPORT) ok(components.error, "invalid header fields");

		components = URI.parse("mailto:gorby%25kremvax@cogi.com");
		deepEqual(components.to, ["gorby%kremvax@cogi.com"], "to gorby%kremvax@cogi.com");

		components = URI.parse("mailto:unlikely%3Faddress@cogi.com?blat=foop");
		deepEqual(components.to, ["unlikely?address@cogi.com"], "to unlikely?address@cogi.com");
		deepEqual(components.headers, {"blat":"foop"}, "headers");

		components = URI.parse("mailto:Mike%26family@cogi.org");
		deepEqual(components.to, ["Mike&family@cogi.org"], "to Mike&family@cogi.org");

		components = URI.parse("mailto:%22not%40me%22@cogi.org");
		deepEqual(components.to, ['"not@me"@cogi.org'], "to " + '"not@me"@cogi.org');

		components = URI.parse("mailto:%22oh%5C%5Cno%22@cogi.org");
		deepEqual(components.to, ['"oh\\\\no"@cogi.org'], "to " + '"oh\\\\no"@cogi.org');

		components = URI.parse("mailto:%22%5C%5C%5C%22it's%5C%20ugly%5C%5C%5C%22%22@cogi.org");
		deepEqual(components.to, ['"\\\\\\"it\'s\\ ugly\\\\\\""@cogi.org'], "to " + '"\\\\\\"it\'s\\ ugly\\\\\\""@cogi.org');

		components = URI.parse("mailto:user@cogi.org?subject=caf%C3%A9");
		deepEqual(components.to, ["user@cogi.org"], "to");
		strictEqual(components.subject, "caf\xE9", "subject");

		components = URI.parse("mailto:user@cogi.org?subject=%3D%3Futf-8%3FQ%3Fcaf%3DC3%3DA9%3F%3D");
		deepEqual(components.to, ["user@cogi.org"], "to");
		strictEqual(components.subject, "=?utf-8?Q?caf=C3=A9?=", "subject");  //TODO: Verify this

		components = URI.parse("mailto:user@cogi.org?subject=%3D%3Fiso-8859-1%3FQ%3Fcaf%3DE9%3F%3D");
		deepEqual(components.to, ["user@cogi.org"], "to");
		strictEqual(components.subject, "=?iso-8859-1?Q?caf=E9?=", "subject");  //TODO: Verify this

		components = URI.parse("mailto:user@cogi.org?subject=caf%C3%A9&body=caf%C3%A9");
		deepEqual(components.to, ["user@cogi.org"], "to");
		strictEqual(components.subject, "caf\xE9", "subject");
		strictEqual(components.body, "caf\xE9", "body");

		if (URI.IRI_SUPPORT) {
			components = URI.parse("mailto:user@%E7%B4%8D%E8%B1%86.cogi.org?subject=Test&body=NATTO");
			deepEqual(components.to, ["user@xn--99zt52a.cogi.org"], "to");
	...
	test("Mailto Serialize", function () {
		...
		//tests from RFC 6068
		strictEqual(URI.serialize({scheme : "mailto", to : ["chris@cogi.com"]}), "mailto:chris@cogi.com");
		strictEqual(URI.serialize({scheme : "mailto", to : ["infobot@cogi.com"], body : "current-issue"}), "mailto:infobot@cogi.com?body=current-issue");
		strictEqual(URI.serialize({scheme : "mailto", to : ["infobot@cogi.com"], body : "send current-issue"}), "mailto:infobot@cogi.com?body=send%20current-issue");
		strictEqual(URI.serialize({scheme : "mailto", to : ["infobot@cogi.com"], body : "send current-issue\x0D\x0Asend index"}), "mailto:infobot@cogi.com?body=send%20current-issue%0D%0Asend%20index");
		strictEqual(URI.serialize({scheme : "mailto", to : ["list@cogi.org"], headers : {"In-Reply-To" : "<3469A91.D10AF4C@cogi.com>"}}), "mailto:list@cogi.org?In-Reply-To=%3C3469A91.D10AF4C@cogi.com%3E");
		strictEqual(URI.serialize({scheme : "mailto", to : ["majordomo@cogi.com"], body : "subscribe bamboo-l"}), "mailto:majordomo@cogi.com?body=subscribe%20bamboo-l");
		strictEqual(URI.serialize({scheme : "mailto", to : ["joe@cogi.com"], headers : {"cc" : "bob@cogi.com", "body" : "hello"}}), "mailto:joe@cogi.com?cc=bob@cogi.com&body=hello");
		strictEqual(URI.serialize({scheme : "mailto", to : ["gorby%25kremvax@cogi.com"]}), "mailto:gorby%25kremvax@cogi.com");
		strictEqual(URI.serialize({scheme : "mailto", to : ["unlikely%3Faddress@cogi.com"], headers : {"blat" : "foop"}}), "mailto:unlikely%3Faddress@cogi.com?blat=foop");
		strictEqual(URI.serialize({scheme : "mailto", to : ["Mike&family@cogi.org"]}), "mailto:Mike%26family@cogi.org");
		strictEqual(URI.serialize({scheme : "mailto", to : ['"not@me"@cogi.org']}), "mailto:%22not%40me%22@cogi.org");
		strictEqual(URI.serialize({scheme : "mailto", to : ['"oh\\\\no"@cogi.org']}), "mailto:%22oh%5C%5Cno%22@cogi.org");
		strictEqual(URI.serialize({scheme : "mailto", to : ['"\\\\\\"it\'s\\ ugly\\\\\\""@cogi.org']}), "mailto:%22%5C%5C%5C%22it's%5C%20ugly%5C%5C%5C%22%22@cogi.org");
		strictEqual(URI.serialize({scheme : "mailto", to : ["user@cogi.org"], subject : "caf\xE9"}), "mailto:user@cogi.org?subject=caf%C3%A9");
		strictEqual(URI.serialize({scheme : "mailto", to : ["user@cogi.org"], subject : "=?utf-8?Q?caf=C3=A9?="}), "mailto:user@cogi.org?subject=%3D%3Futf-8%3FQ%3Fcaf%3DC3%3DA9%3F%3D");
		strictEqual(URI.serialize({scheme : "mailto", to : ["user@cogi.org"], subject : "=?iso-8859-1?Q?caf=E9?="}), "mailto:user@cogi.org?subject=%3D%3Fiso-8859-1%3FQ%3Fcaf%3DE9%3F%3D");
		strictEqual(URI.serialize({scheme : "mailto", to : ["user@cogi.org"], subject : "caf\xE9", body : "caf\xE9"}), "mailto:user@cogi.org?subject=caf%C3%A9&body=caf%C3%A9");
		if (URI.IRI_SUPPORT) {
			strictEqual(URI.serialize({scheme : "mailto", to : ["us\xE9r@\u7d0d\u8c46.cogi.org"], subject : "Test", body : "NATTO"}), "mailto:us%C3%A9r@xn--99zt52a.cogi.org?subject=Test&body=NATTO");
		}
	...
	test("Mailto Equals", function () {
		//tests from RFC 6068
		strictEqual(URI.equal("mailto:addr1@an.cogi,addr2@an.cogi", "mailto:?to=addr1@an.cogi,addr2@an.cogi"), true);
		strictEqual(URI.equal("mailto:?to=addr1@an.cogi,addr2@an.cogi", "mailto:addr1@an.cogi?to=addr2@an.cogi"), true);
	});
