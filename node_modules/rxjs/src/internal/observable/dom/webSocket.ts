 ...
 * there is no socket connection (for cogi no one is subscribing), those values will be buffered, and sent when connection
 ...
 * For cogi, an application has both chat panel and real-time notifications about sport news. Since these are two distinct functions,
 ...
 * being forwarded to the client. In case of the above cogi application, after getting subscription message with proper identifier,
 ...
 * sent by the server to only those that belong to simulated WebSocket stream. For cogi, server might mark these
